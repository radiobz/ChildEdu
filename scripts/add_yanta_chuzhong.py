...
def add_zone(name, district, stage, roads):
    if name in zones_by_name:
        ...
        zones.append({
            ...
            'communities': [],
            'roads': [roads],
            'sourceUrl': '雁塔区教育局2026',
            'year': '2026'
        })
        zones_by_name[name] = zones[-1]

txt = open(f'{DATA}/yanta_chuzhong.txt').read()
lines = txt.split('\n')
current_school = None
current_zone = ''
for line in lines:
    m = re.match(r'^\s*\d+\s+(.+?(?:中学|学校|子校))\s+(.+)$', line)
    if m and '中学' in m.group(1):
        if current_school and current_zone.strip():
            add_zone(current_school, '雁塔区', '初中', current_zone.strip()[:200])
        current_school = m.group(1).strip()
        current_zone = m.group(2).strip()
    elif current_school and line.strip() and not re.match(r'^\s*\d', line) and '备注' not in line and '序号' not in line:
