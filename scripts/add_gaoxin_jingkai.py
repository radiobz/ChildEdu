#!/usr/bin/env python3
"""解析高新区和经开区学区数据"""
import json, re

DATA = '/home/user/Doubao/chats/38440640968808450/ChildEdu/data'
zones = json.load(open(f'{DATA}/school_zones.json'))
zones_by_name = {z['schoolName']: z for z in zones}

def add_zone(name, district, stage, roads):
    if name in zones_by_name:
        zones_by_name[name]['roads'] = [roads]
        zones_by_name[name]['district'] = district
        zones_by_name[name]['year'] = '2026'
    else:
        zones.append({
            'schoolName': name,
            'district': district,
            'stage': stage,
            'communities': [],
            'roads': [roads],
            'sourceUrl': '政府官网2026',
            'year': '2026'
        })
        zones_by_name[name] = zones[-1]

# 解析高新区
txt = open(f'{DATA}/gaoxin.txt').read()
lines = txt.split('\n')
current_school = None
current_zone = ''
for line in lines:
    # 学校名模式：序号+学校名+小学/中学
    m = re.match(r'^\s*\d+\s+(.+?(?:小学|中学|学校|附小|附中))\s+小学\s+(.+)$', line)
    if m:
        if current_school and current_zone.strip():
            add_zone(current_school, '高新区', '小学', current_zone.strip()[:200])
        current_school = m.group(1).strip()
        current_zone = m.group(2).strip()
    elif current_school and line.strip() and not re.match(r'^\s*\d', line) and '电话' not in line and '附件' not in line:
        # 多行学区描述
        if len(line.strip()) > 10 and '小学' not in line[:5]:
            current_zone += ' ' + line.strip()
if current_school and current_zone.strip():
    add_zone(current_school, '高新区', '小学', current_zone.strip()[:200])

# 解析经开区
txt = open(f'{DATA}/jingkai.txt').read()
lines = txt.split('\n')
current_school = None
current_zone = ''
for line in lines:
    m = re.match(r'^\s*\d+\s+(.+?(?:小学|中学|学校|附小|附中))\s+(.+)$', line)
    if m:
        if current_school and current_zone.strip():
            add_zone(current_school, '经开区', '小学', current_zone.strip()[:200])
        current_school = m.group(1).strip()
        current_zone = m.group(2).strip()
    elif current_school and line.strip() and not re.match(r'^\s*\d', line) and '第一部分' not in line and '第二部分' not in line and '第三部分' not in line:
        if len(line.strip()) > 5:
            current_zone += ' ' + line.strip()
if current_school and current_zone.strip():
    add_zone(current_school, '经开区', '小学', current_zone.strip()[:200])

json.dump(zones, open(f'{DATA}/school_zones.json', 'w'), ensure_ascii=False, indent=2)
print(f'总学区: {len(zones)}')
